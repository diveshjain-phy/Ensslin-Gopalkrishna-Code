...
# chisqr calculation
def chisqr(flx_obs,flx_norm,f_err):
	xchisqr=0
	for i in range(len(flx_obs)):
#		print(math.log(flx_obs[i]))
	
#		print(flx_norm[i])
#		print(math.log(f_err[i]))
		if flx_norm[i]==0:
			flx_norm[i]=1e-100		
#		print(((math.log(flx_obs[i]/1e-26)-math.log(flx_norm[i]/1e-26))/math.log(f_err[i]/1e-26))**2)
		xchisqr=xchisqr+((math.log(flx_obs[i]/1e-26,10)-math.log(flx_norm[i]/1e-26,10))/math.log(f_err[i]/1e-26,10))**2
	return xchisqr
...
def chicalc(del_t,tau,ph,z,B_src,V_src,b,f_nuobs,f_err,flx_obs,alphaex):
...
##	#	normalizing calculated result
	...
	chi	=0
	#################### chi sqr calculation ###################
	chi=chisqr(flx_obs,flx_norm,f_err)
	return chi,flx_norm,norm,B,V	
