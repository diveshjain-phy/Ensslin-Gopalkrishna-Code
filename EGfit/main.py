...
for alphaex in nalphae:
	...
	for i in timeex:
		...
		delt[4]	=i['delt4']	
##################################################################
# user may also vary magnetic field for better estimation
		Bsrc	=i['Bsrc']
		B_src	=Bsrc*1e-8
		print("Current choice of magnetic field",B_src,"mu G")
###################################################################
...
# Obtaining minimum of Chi and hence the corresponding parameters
	chi_min_ind	=np.nanargmin(chilist)
	chi_min		=chilist[chi_min_ind]
	clist.append(chi_min)
	flx_min		=flx[chi_min_ind]
	norm_min	=nm[chi_min_ind]
	mag_min		=magB[chi_min_ind]
	vol_min		=volV[chi_min_ind]
	mag_min	=[i/1e-6 for i in mag_min]
	vol_min	=[i/(3.08*1e24)**3 for i in vol_min]
	vlist.append(vol_min[1])
	# instancing time scale for each set
