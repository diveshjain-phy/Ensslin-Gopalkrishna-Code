...
# Assigning the variables to be passed
	...
	# Assigning Magnetic Energy Density and Magnetic Field at each stfluxep
	B_x[ph]		=B_src
...
def chicalc(del_t,tau,ph,z,B_src,V_src,b,f_nuobs,f_err,flx_obs,norm):
# Input Parameter #
#del_t	:Duration of phase		List	
#tau	:Timescale of expansion		List
#ph	:user input for phase		integer
...
###	# Calculating Luminosity and Flux received by the observer
	...
	Vi	=V[ph]
	for nu in f_nuobs:
#			L_freq	=c3*Bi*Vi*formula(10,pstar0i,10000,nu,Bi,C0i,alphae,pstar0i)
			L_freq	=c3*Bi*Vi*quad(integrand,10,pstar0i,args=(nu,Bi,C0i,alphae,pstar0i))[0]
			if d==0:
				flx_cal.append(L_freq)
			else:
				flx_cal.append(L_freq/(4*pi*d**2))

	flx_cal	=[i*norm for i in flx_cal]

	chi=0	#redundant parameter
	return chi,flx_cal,B,u_B,C,C0,V,pstar0i
